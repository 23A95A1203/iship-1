...
document
  .getElementById("calculateNutrition")
  .addEventListener("click", function () {
    ...
    ingredients.forEach((ingredient) => {
      const quantity =
        parseFloat(document.getElementById(ingredient)?.value) || 0;
      if (quantity > 0) {
    ...
    document.getElementById(
      "calories"
    ).textContent = `Calories:${totalCalories.toFixed(2)} kcal`;
    document.getElementById(
      "protein"
    ).textContent = `Protein: ${totalProtein.toFixed(2)} g`;
    document.getElementById(
      "carbs"
    ).textContent = `Carbohydrates: ${totalCarbs.toFixed(2)} g`;
    document.getElementById("fats").textContent = `Fats: ${totalFats.toFixed(
      2
    )} g`;
  });
